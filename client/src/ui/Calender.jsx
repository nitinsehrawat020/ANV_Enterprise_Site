...
function Calender({ datesData = [] }) {
  const [currentDate, setCurrentDate] = useState(new Date());
  const [calendarDays, setCalendarDays] = useState([]);

  // Convert datesData to a map for easy lookup
  const datesMap = datesData.reduce((acc, item) => {
    if (shouldAddItemToMap(item)) {
      acc[item.date] = item.status;
    }
    return acc;
  }, {});

  function shouldAddItemToMap(item) {
    return item && item.date && item.status;
  }
          ...
          <DaysGrid>
            {calendarDays.map((day, index) => (
              <DayCell
                ...
                title={
                  day.status
                    ? `${formatDateDDMMYYYY(day.fullDate)}: ${day.status}`
